...
"""Pytest configuration and fixtures for shared tests.
...
@pytest.fixture
def regular_user(user_factory: type[UserFactory]) -> User:
    """Fixture creating a regular user for testing.

    Returns:
        User: A regular user instance (not staff, not superuser).
    """
...
def staff_user(user_factory: type[UserFactory]) -> User:
    """Fixture creating a staff user for testing.

    Returns:
        User: A staff user instance.
    """
    return user_factory.create(
        is_superuser=False,
        is_staff=True,
